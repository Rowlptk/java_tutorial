...
// class and return 0 if the values are equal, greater than zero if the value sorts greater than
...
//  If the item to be added is less tthan the current item in the tree, add the item before the
...
//      agin (if there is no next item then that is where the new item belongs).
//
...
// classes that we create later, but if you use names that Intellij(or Netbeans etc.) recognises as Java classes (such
...
//
// Hint: to avoid typing loads od "addItem" lines, split a string into an array and create your tree in
